---
title: DL 10/2024 — Simplex (End of Municipal Inspection)
type: entity
tags: [dl-10-2024, phase-4, inspection]
status: draft
created: 2026-07-13
updated: 2026-07-20
sources: [src-construction-defects-prompt-chain]
brand: shared
verified: false
---

# DL 10/2024 — Simplex

The urbanistic simplification package. Its operative consequence for this venture: **the state no longer performs technical inspection of new builds** — licensing shifted toward self-certification by the developer's own technicians (*termos de responsabilidade*).

## Why it creates the inspection market

- The Câmara Municipal lacks (and no longer deploys) specialized engineers to physically verify construction — enabling the self-certification loopholes documented across the defect modules (e.g., SCIE material substitution, acoustic pre-cert bait-and-switch).
- "Value engineering" on 2024+ builds is now caught by nobody unless the buyer hires a private *perito*. Private inspection moved from optional comfort to the only verification layer that exists.

## Where it appears

[[phase-4-due-diligence|Phase 4]] framing · [[platform-home-inspection|inspection platform thesis]] · [[scie-fire-safety]] and [[acoustic-defects]] fraud patterns · content angle: "Simplex: why the government no longer inspects your new apartment" and the "Simplex Survival Guide" [[lead-magnets|lead magnet]].

## Superseded/extended by DL 108/2026

[[dl-108-2026|DL 108/2026]] (in force 3 August 2026) is the second phase of the same Simplex reform, not a contradicting law — where this decree removed state *technical* inspection of new builds, DL 108/2026 removes state *documentary* verification at the point of sale too (the habitation license is no longer required to execute a deed). Read this page as "phase one" of a still-unfolding deregulation; DL 108/2026 is where the thesis sharpens.
