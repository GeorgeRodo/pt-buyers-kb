---
title: Home — Portugal Buyer-Side Knowledge Base
type: hub
tags: [meta]
status: evergreen
created: 2026-07-13
updated: 2026-07-23
sources: [src-buyers-agent-playbook-2026, src-construction-defects-prompt-chain]
brand: shared
verified: false
---

# Portugal buyer-side knowledge base

One venture, one thesis: **the Portuguese real estate market is structurally tilted toward the seller, and the buyer's side is underserved.** The listing agent works for the seller (DL 118/2013), the state stopped inspecting new builds (*Simplex*, DL 10/2024), and hidden-defect liability now runs on brutal 30-day clocks (DL 67/2023). Every product this venture builds sells protection inside that gap.

## The logic of the project

```
        THE BUYER JOURNEY (the ecosystem, decomposed)
  P1 → P2 → P3 → P4 → P5 → P6 → P7 → P8
  onboard sourcing viewings DUE-DIL finance offer/CPCV closing post-purchase
                     │        │
                     │        └── inspection platform lives HERE
                     │
        ┌────────────┴─────────────┐
        ▼                          ▼
  PLATFORM 1                 PLATFORM 2
  Buyer's Agency             Home Inspection
  (spans P1–P8)              (P3–P4 + post-CPCV)
        ▲                          ▲
        └───────── fed by ─────────┘
  DOMAIN KNOWLEDGE: legal · construction pathology · market intel · regional geology
```

Home inspection is deliberately **not** the root of this knowledge base. The buyer's side is decomposed first; inspection emerges as the technical due-diligence slice of that decomposition. See [[inspection-in-the-buyer-journey|why inspection is a subset, not the frame]].

## Navigate

### The spine
- [[buyer-journey|The buyer journey]] — hub for all 8 phases
- Phases: [[phase-1-onboarding|1 Onboarding]] · [[phase-2-sourcing|2 Sourcing]] · [[phase-3-viewings|3 Viewings]] · [[phase-4-due-diligence|4 Due diligence]] · [[phase-5-financing|5 Financing]] · [[phase-6-offer-cpcv|6 Offer & CPCV]] · [[phase-7-closing|7 Closing]] · [[phase-8-post-purchase|8 Post-purchase]]

### The money
- [[platforms-overview|Platforms overview]] — how the platforms map to the journey
- [[platform-buyers-agency|Buyer's agency]] · [[platform-home-inspection|Home inspection]]
- [[monetization-strategy|Monetization strategy]] · [[content-strategy|Content strategy]] · [[lead-magnets|Lead magnets]] · [[b2b-partnerships|B2B partnerships]] · [[sops|SOPs]]
- [[hiring-hub|Hiring & inspector operations]] — internal company-building content: staffing the inspection platform (not buyer-facing)
- [[ops-role-hub|Growth & operations role]] — internal company-building content: SEO ownership, workspace access/credentials, tooling & billing for InspectOS + HomeOS (not buyer-facing)
- [[inspectos-pca-pcs-strategy|InspectOS PCA/PCS strategy]] — the three-tier standard-setting architecture behind [[platform-home-inspection]]
- [[inspectos-global-benchmarks|Global inspection benchmarks]] — pricing, specialist-niche economics, and Portugal's #1 market-entry ranking
- [[inspectos-b2b-ecosystem|InspectOS B2B ecosystem]] — lender/insurer/PropTech distribution strategy; Portugal currently has zero formal B2B inspection partnerships

### The knowledge
- Legal: [[legal-hub|Legal hub]] — [[dl-67-2023|DL 67/2023 hidden defects]] · [[dl-10-2024-simplex|DL 10/2024 Simplex]] · [[cpcv|CPCV]] · [[condominio-law|Condomínio law]]
- Construction: [[construction-defects-hub|Defects hub]] — [[acoustic-defects|Acoustics]] · [[seismic-risk|Seismic]] · [[scie-fire-safety|Fire/SCIE]] · [[instagram-renovation-detection|Flip detection]]
- Market: [[market-intelligence|Market intelligence]] · [[sourcing-strategy|Sourcing strategy]] · [[comparables|Comparables]] · [[property-types-hub|Property types]]
- Regions: [[regions-hub|Regions hub]] — [[lisbon|Lisbon]] · [[porto|Porto]] · [[algarve|Algarve]]
- [[entities-hub|Entities]] — institutions, portals, registries (competing inspection firms, mediators, and buyer's agents file here too, tagged `competitor`)
- [[syntheses-hub|Syntheses]] — filed answers and analyses, incl. the [[open-decisions-tracker|open decisions tracker]]

### Personas
- [[personas-hub|Personas hub]] — how buyer segments experience the journey differently
- [[d7-expat|D7 expat]] — seed persona (stub)

### Housekeeping
- [[index|Index]] — full page catalog · `log.md` — activity log · `CLAUDE.md` — schema · `README.md` — usage instructions
- `09-output/` — Exgest drafts (pSEO, lead magnets, social, email); not part of the wiki graph, created on first Exgest run
